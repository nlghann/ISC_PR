...
export default function BatchProductionPlanModal({
  ...
  const [rowDates, setRowDates] = useState<Record<string, { startDate: string; endDate: string }>>({});

  const handleSelectRow = (req: SelectedRequest) => {
    setSelectedDetail(req);
  };

  const getRowDate = (req: SelectedRequest) =>
    rowDates[req.id] ?? { startDate: req.startDate || '', endDate: req.endDate || '' };
  ...
  const updateRowDate = (
    ...
    setRowDates((prev) => {
      const current = prev[id] ?? { startDate: req.startDate || '', endDate: req.endDate || '' };
                  ...
                  {selectedRequests.map((req, idx) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="date"
                          value={getRowDate(req).startDate}
                          onChange={(e) => updateRowDate(req.id, 'startDate', e.target.value, req)}
                          className="px-2 py-1 text-xs text-slate-900 bg-white border border-blue-400 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      </td>
                      <td className="px-3 py-2 text-xs" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="date"
                          value={getRowDate(req).endDate}
                          onChange={(e) => updateRowDate(req.id, 'endDate', e.target.value, req)}
                          className="px-2 py-1 text-xs text-slate-900 bg-white border border-blue-400 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      </td>
